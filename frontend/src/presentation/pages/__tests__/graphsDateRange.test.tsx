  ...
  return {
    ...
    DatePicker: {
      RangePicker: ({
        ...
        onChange?: (
          values: Array<{
            startOf: () => { toISOString: () => string };
            endOf: () => { toISOString: () => string };
          }> | null,
        ) => void;
      }) => (
        <div>
              ...
              onChange?.([
                {
                  ...
                  endOf: () => ({
                    toISOString: () => "2026-08-20T23:59:59.999Z",
                  }),
                },
                {
                  startOf: () => ({
                  ...
                  endOf: () => ({
                    toISOString: () => "2026-08-21T23:59:59.999Z",
                  }),
                },
              ])
            }
...
mockUseOntTrafficTimeSeries.mockReturnValue({
  isLoading: false,
  data: [{ time: new Date("2026-08-20T12:00:00.000Z"), txMbps: 10, rxMbps: 5 }],
});
...
function cardWithSerial(serial: string) {
  return screen.queryByText((_, element) => {
    ...
    return (
      className.includes("ant-card-body") &&
      element?.textContent?.includes(serial) === true
    );
  });
}
...
describe("GraphsPage date range", () => {
  it("displays ONT status tag on each traffic card", () => {
    ...
    expect(
      screen.getByText(
        (_, element) =>
          element?.className?.toString().includes("ant-tag") === true &&
