...
import { Timeline, Typography, Empty, Spin, Tag, Card, Statistic, Row, Col, Select } from 'antd';
...
export const ONTEventTimeline: React.FC<ONTEventTimelineProps> = ({ ontId, days: initialDays = 7 }) => {
  const [selectedDays, setSelectedDays] = useState(initialDays);
  const { data: eventsData, isLoading: eventsLoading } = useOntEvents(ontId, 100, 0);
  const { data: availabilityData, isLoading: availabilityLoading } = useOntAvailability(ontId, selectedDays);
  ...
  return (
    <div>
      <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={5} style={{ margin: 0 }}>Availability Statistics</Title>
        <Select
          value={selectedDays}
          ...
          <Row gutter={16}>
            <Col span={6}>
              <Statistic
                ...
                precision={2}
                suffix="%"
                valueStyle={{ color: availabilityData.availabilityPercent >= 99 ? '#3f8600' : '#cf1322' }}
              />
            </Col>
            <Col span={6}>
              <Statistic
            ...
            <Col span={6}>
              <Statistic
                ...
                value={availabilityData.mtbf > 0 ? formatDuration(availabilityData.mtbf) : 'N/A'}
              />
            </Col>
            <Col span={6}>
              <Statistic
                title="MTTR (Mean Time To Repair)"
                value={availabilityData.mttr > 0 ? formatDuration(availabilityData.mttr) : 'N/A'}
              />
            </Col>
          </Row>
      ...
      <Title level={5} style={{ marginBottom: '16px' }}>Event History</Title>
