...
func TestDrainSendsTheWhatsAppIDOfTheQuotedMessage(t *testing.T) {
	...
	sender := &fakeSender{}
	sent, err := NewDrainer(messages, conversations, sender, t.TempDir(), 0).Drain(context.Background(), 10)
	require.NoError(t, err)
...
func TestDrainMarksAQuoteOfOurOwnReplyAsOurs(t *testing.T) {
	...
	sender := &fakeSender{}
	_, err = NewDrainer(messages, conversations, sender, t.TempDir(), 0).Drain(context.Background(), 10)
	require.NoError(t, err)
