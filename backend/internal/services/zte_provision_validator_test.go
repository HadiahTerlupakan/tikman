...
func TestValidateZTEGPONRegister(t *testing.T) {
	tests := []struct {
		...
		{name: "rejects unsupported WAN mode", mutate: func(req *models.ZTEGPONRegisterRequest, olt *models.OLT) { req.WANMode = "ipoe" }, wantSubstr: "wan_ip or setup_via_ont"},
		{name: "rejects unsupported WAN-IP mode", mutate: func(req *models.ZTEGPONRegisterRequest, olt *models.OLT) { req.WANIPMode = "ipoe" }, wantSubstr: "pppoe, dhcp or static"},
